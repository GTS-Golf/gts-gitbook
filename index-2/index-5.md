...
이 문서는 텐서플로우 서버 클러스터를 생성하고, 클러스터 상에서 분산 처리를 수행하는 방법에 대하여 설명한다. 이 문서의 독자들은 텐서플로우를 이용한 [기본적인 프로그래밍 개념](../g3doc/get\_started/basic\_usage/)은 숙지가 되어있다고 가정한다 .
...
[`tf.train.Server.create_local_server()`](../index-3/index/train.md#Server.create\_local\_server) 메소드는 in-process 서버 형태로 단일 프로세스 클러스터를 생성한다.
...
[`tf.train.Server`](../index-3/index/train.md#Server) 객체는 여러개의 로컬 디바이스 정보와, 각 작업과 디바이스를 연결해주는 정보인 `tf.train.ClusterSpec` , 분산 연산 수행에 이용되는 ["session target"](../index-3/index/client.md#Session) 을 포함하고 있다.
...
연산을 CPU 혹은 GPU에서 수행할지 선택할 때와 마찬가지로, 특정 연산에 연산을 수행할 디바이스를 명시할 때는 [`tf.device()`](../index-3/index/framework.md#device) 를 사용하면 된다.
